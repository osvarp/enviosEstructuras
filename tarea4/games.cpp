...
/*
Se agrego el uso de la libreria string para representar las cadenas de esta manera
*/
...
void leer();
void modQuickSort(int * medioBajo,int * medioAlto, string names[MaxNames], int n, int * listaMenor, int * listaIgual, int * listaMayor, int * lis);

void sortInLists(string names[MaxNames],int * lis, int n, int * listaMenor, int * lenMenor, int * listaIgual, int * lenIgual, int * listaMayor,int*lenMayor);
void solution(string names[MaxNames], int n, int * listaMenor, int * listaIgual, int * listaMayor, int * lis);
void printNFindMiddlePoint(string bajo, string alto, int n);
...
void printNFindMiddlePoint(string bajo, string alto, int n){
	/*cout << bajo << " " << alto<<endl;*/
	int flag = 0;
	string::iterator i = bajo.begin(), j = alto.begin();
	while (flag == 0 && i != bajo.end()){
		if (*i != *j && ((*i)+1 != *j || (j+1) != alto.end()) && *i != 'Z'){
			flag = 1;
		} else{
			cout << *i;
			++i;
...
void sortInLists(string names[MaxNames], int * lis, int n, int * listaMenor, int * lenMenor, int * listaIgual, int * lenIgual, int * listaMayor,int*lenMayor){
	...
	*lenMayor = 0;
	for (i = 1; i < n; ++i){
		if (names[listaIgual[0]] == names[lis[i]]){
			listaIgual[(*lenIgual)++] = lis[i];
		} else if(names[listaIgual[0]] > names[lis[i]]){
